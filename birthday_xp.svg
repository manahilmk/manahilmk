...
max_days = 365  # or 366 for leap year
...
days_since_birthday = (today - date(today.year if today < next_birthday else today.year -1, dob.month, dob.day)).days
days_until_next = (next_birthday - date(today.year if today < next_birthday else today.year -1, dob.month, dob.day)).days
xp_percent = days_since_birthday / days_until_next
...
<svg width="460" height="64" viewBox="0 0 460 64" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- XP text inside bar -->
  <text x="210" y="36" font-family="Georgia, 'Times New Roman', serif" font-size="13" fill="#fff8e6" text-anchor="middle">{days_since_birthday} / {days_until_next} XP</text>
...
# save to file
with open("birthday_xp.svg", "w") as f:
